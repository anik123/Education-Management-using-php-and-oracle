...
include("connection.php");
 ...
 if(!isset($_SESSION["user"]))
		 ...
		 */
		 ...
		 {
			 $exam=$_GET["VideoId"];
			/* $data=oci_parse($conn,"select a.CourseCode from assignstudent a ,upload u where u.CourseCode=a.CourseCode and u.UploadId=$exam");
			 oci_execute($data);
			 $data=oci_num_rows($data);
			 if($data<=0)
			 {
				  echo "Invalid Page<br>";
		   echo "<a href='home.php'> <<<<< Back <<<<<< </a>";
...
<script type="text/javascript">
  function change(){
	...
	http.open("POST","GetVideoList.php",false);
...
if($query["ROLE"]=="Admin")
   echo "<h3>Notice</h3>
	...
	<section id="main" class="column">
        ...
        <?php
			...
			oci_execute($query);
			$query=oci_fetch_array($query,OCI_ASSOC+OCI_RETURN_NULLS);
			$path="upload/".$query["UPLOADBY"]."/";
			$path=$path . basename($query["UPLOADFILE"]);
		 ...
		 $c="upload/";
			   if(strcmp($c,$path)!=0){
               echo "<video src='".$path."' width='700' height='400' controls preload></video>";
			   }
			   else
			   echo "<video src='' width='700' height='400' controls preload></video>";
  ...
  ?>
        ...
        <div class="spacer"></div>
        <article class="module width_full">
			<header><h3>Video Files</h3></header>
				<div class="module_content">
					<span id="cv"> </span>
					</ul>
				</div>
