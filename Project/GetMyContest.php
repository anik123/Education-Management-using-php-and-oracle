...
$course=$_POST["course"];
	...
	echo "<header><h3 class='tabs_involved'>My Contest List</h3>
		</header>
	...
	<th><center>Created By</center></th>
        ...
        ";   
		if($course=="0")
		$data=mysql_query("select * from contest where CreatedBy='".$_SESSION["user"]."'");
		else if($course=="1")
		$data=mysql_query("select * from contest where ContestDate<'".date("Y-m-d")."' and CreatedBy='".$_SESSION["user"]."'");
		else if($course=="2")
		$data=mysql_query("select * from contest where ContestDate='".date("Y-m-d")."' and CreatedBy='".$_SESSION["user"]."'");
		else if($course=="3")
		$data=mysql_query("select * from contest where ContestDate>'".date("Y-m-d")."' and CreatedBy='".$_SESSION["user"]."'");
